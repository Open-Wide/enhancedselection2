...
class SckEnhancedSelectionType extends eZDataType {
	...
	function fetchClassAttributeHTTPInput( $http, $base, $classAttribute ) {
		...
		$identifierArrayName = join( '_', array( $base, 'sckenhancedselection_identifier',
			$id ) );
		$priorityArrayName = join( '_', array( $base, 'sckenhancedselection_priority',
			$id ) );
			...
			case 'sort_optionlist': {
					...
					if ( $http->hasPostVariable( $sortName ) ) {
						...
						if ( strpos( $sort, '_' ) !== false ) {
							...
							$priorityArray = array();
							if ( $type == 'prior' ) {
								$priorityArray = $http->postVariable( join( '_', array( $base, 'sckenhancedselection_priority',
									$id ) ) );
							}
